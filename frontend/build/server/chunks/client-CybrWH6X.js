const PUBLIC_API_URL = "https://31.57.187.149";
...
api.users = {
  ...
  search: async (q) => {
    ...
    const data = await unwrap(api.get(`/users/search?q=${encodeURIComponent(q)}`));
...
api.posts = {
  ...
  get: async (id) => unwrap(postApi.getPost(id)),
  getComments: async (id, page = 1) => {
    try {
      const data = await unwrap(postApi.getComments(id, page));
      return { items: Array.isArray(data) ? data : data?.items ?? data?.comments ?? [] };
    } catch {
      return { items: [] };
    }
  },
  addComment: (id, content) => unwrap(postApi.addComment(id, content)),
...
const subscriptionApi = {
  subscribe: (username, data) => api.post(`/subscriptions/subscribe/${username}`, data),
  unsubscribe: (username) => api.delete(`/subscriptions/unsubscribe/${username}`),
  checkSubscription: (username) => api.get(`/subscriptions/check/${username}`),
  ...
  getPlans: (username) => api.get(`/subscriptions/users/${username}/plans`),
...
api.payments = {
  tip: (recipientId, amount, opts) => unwrap(
  ...
  unlockPost: (postId) => unwrap(api.post(`/payments/posts/${postId}/unlock`, { payment_method: "wallet" })),
  createDeposit: (amount, method = "monero") => unwrap(api.post("/payments/deposit", { amount, payment_method: method })),
  checkStatus: (id) => unwrap(api.get(`/payments/request/${id}/status`))
};
api.subscriptions = {
  getCreatorPlans: async (username) => {
    try {
      const data = await unwrap(subscriptionApi.getPlans(username));
      return Array.isArray(data) ? data : data?.items ?? data?.plans ?? [];
    } catch {
      return [];
    }
  },
  subscribe: async (creatorId, username, opts) => unwrap(
  ...
  unsubscribe: (username) => unwrap(subscriptionApi.unsubscribe(username))
};
api.wallet = {
  get: async () => {
...
api.reports = {
  create: (data) => unwrap(api.post("/reports", { type: "other", description: "", ...data })),
  queue: (statusFilter) => unwrap(api.get(`/reports/queue${statusFilter ? `?status_filter=${statusFilter}` : ""}`)),
  resolve: (id, data) => unwrap(api.post(`/reports/${id}/resolve`, data))
};
api.ads = {
  list: async (position) => {
    try {
      const data = await unwrap(api.get(`/ads?position=${position}`));
      return data?.items ?? [];
    } catch {
      return [];
    }
  },
  click: (id) => unwrap(api.post(`/ads/${id}/click`)),
  all: () => unwrap(api.get("/ads/all")),
  create: (data) => unwrap(api.post("/ads", data)),
  update: (id, data) => unwrap(api.put(`/ads/${id}`, data)),
  remove: (id) => unwrap(api.delete(`/ads/${id}`))
};
api.tickets = {
  create: (data) => unwrap(api.post("/tickets", data)),
  mine: async () => {
    const d = await unwrap(api.get("/tickets"));
    return d?.items ?? [];
  },
  get: (id) => unwrap(api.get(`/tickets/${id}`)),
  reply: (id, text) => unwrap(api.post(`/tickets/${id}/reply`, { text })),
  close: (id) => unwrap(api.post(`/tickets/${id}/close`)),
  queue: async (statusFilter) => {
    const d = await unwrap(api.get(`/tickets/queue${statusFilter ? `?status_filter=${statusFilter}` : ""}`));
    return d?.items ?? [];
  }
};
api.escrow = {
  create: (data) => unwrap(api.post("/escrow", data)),
  mine: async (role = "all") => {
    const d = await unwrap(api.get(`/escrow?role=${role}`));
    return d?.items ?? [];
  },
  deliver: (id, data) => unwrap(api.post(`/escrow/${id}/deliver`, data)),
  approve: (id) => unwrap(api.post(`/escrow/${id}/approve`)),
  cancel: (id) => unwrap(api.post(`/escrow/${id}/cancel`)),
  dispute: (id, reason) => unwrap(api.post(`/escrow/${id}/dispute`, { reason })),
  resolve: (id, action) => unwrap(api.post(`/escrow/${id}/resolve?action=${action}`))
};
api.flagScreenshot = () => unwrap(api.post("/users/me/flag-screenshot"));

export { api as a, authApi as b, userApi as u };
//# sourceMappingURL=client-CybrWH6X.js.map
