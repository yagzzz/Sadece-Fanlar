import { c as create_ssr_component, d as add_attribute, a as subscribe, b as createEventDispatcher, f as escape, v as validate_component, e as each } from './ssr-CAqK4lyn.js';
import { c as cn, t as timeAgo, f as formatCurrency } from './index2-O1tI1Vis.js';
...
const PostCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<article${add_attribute("class", cn("bg-white dark:bg-neutral-900 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800 overflow-hidden", className), 0)}> ${showCreator ? `<div class="p-4 flex items-center justify-between"><a href="${"/" + escape(post.user?.username, true)}" class="flex items-center gap-3 group">${validate_component(Avatar, "Avatar").$$render(
    $$result,
    {
    ...
    {},
    {}
  )} <div><div class="flex items-center gap-1"><span class="font-semibold text-neutral-900 dark:text-white group-hover:text-primary transition-colors">${escape(post.user?.display_name)}</span> ${post.user?.is_verified ? `${validate_component(Badge, "Badge").$$render($$result, { variant: "primary", class: "text-xs" }, {}, {
    default: () => {
      return `✓`;
    }
  })}` : ``}</div> <p class="text-sm text-neutral-500">@${escape(post.user?.username)} · ${escape(timeAgo(post.created_at))}</p></div></a> ${validate_component(Dropdown, "Dropdown").$$render($$result, { items: dropdownItems }, {}, {})}</div>` : ``}  ${post.content ? `<div class="px-4 pb-3"><p class="text-neutral-800 dark:text-neutral-200 whitespace-pre-wrap">${escape(post.content)}</p></div>` : ``}  ${post.media && post.media.length > 0 ? `<div class="relative aspect-square bg-neutral-100 dark:bg-neutral-800">${isLocked ? ` <div class="absolute inset-0 flex flex-col items-center justify-center bg-neutral-900/80 backdrop-blur-xl"><div class="text-4xl mb-4" data-svelte-h="svelte-lmppts">🔒</div> <p class="text-white text-lg font-medium mb-2" data-svelte-h="svelte-yuvg0d">Premium İçerik</p> <p class="text-neutral-400 mb-4">${escape(formatCurrency(post.price || 0))} ile kilidi aç</p> ${validate_component(Button, "Button").$$render($$result, {}, {}, {
    default: () => {
      return `Kilidi Aç`;
    }
...
//# sourceMappingURL=PostCard-CTvdy8Cn.js.map
